      ...
      <section className="py-24 max-w-7xl mx-auto px-6">
                ...
                <button
                  ...
                  className={`px-6 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all duration-500 ${
                    activeTab === cat 
                      ...
                      : "bg-white/[0.03] text-slate-500 hover:bg-white/10 hover:text-white border border-white/5"
                  }`}
                >
                  {cat}
                </button>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {filteredItems.map((item, i) => (
            <div 
              key={i} 
              className="group relative aspect-square rounded-[2.5rem] overflow-hidden border border-white/10 bg-white/[0.03] animate-fadeIn shadow-2xl"
            ...
            >
              ...
              <div className="absolute bottom-0 left-0 right-0 p-8 translate-y-4 group-hover:translate-y-0 transition-all duration-700">
                ...
                <h4 className="text-sm font-black text-white tracking-tight mt-3 leading-snug">{item.title}</h4>
              </div>
        ...
        {filteredItems.length === 0 && (
          <div className="py-40 text-center space-y-6">
             <div className="w-24 h-24 bg-white/[0.03] border border-white/5 rounded-full flex items-center justify-center mx-auto text-slate-800 animate-pulse">
                <MdImage size={48} />
             </div>
             <p className="text-slate-500 font-black uppercase tracking-[0.3em] text-[10px]">No memories found in this category</p>
          </div>
        )}
      </section>
