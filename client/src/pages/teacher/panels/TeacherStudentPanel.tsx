...
import { FiMail, FiPhone, FiX, FiCalendar, FiUsers, FiSearch, FiChevronDown, FiUser, FiHash, FiTarget, FiEye, FiActivity, FiHeart, FiBriefcase, FiMapPin, FiRefreshCw } from "react-icons/fi";
import { useToast } from "../../../context/ToastContext";
import { useTeacher } from "../../../context/TeacherContext";
import type { TeacherStudent, TeacherClassAssignment } from "../../../types/teacher/types";

const ViewStudentModal = ({ student, onClose }: { student: TeacherStudent, onClose: () => void }) => {
...
const TeacherStudentPanel = () => {
  ...
  useEffect(() => {
    api.get("/teacher/assignments/my").then(res => {
      const data: TeacherClassAssignment[] = res.data.data || [];
  ...
  useEffect(() => {
    if (selectedClassId) {
      api.get("/teacher/assignments/my").then(res => {
        const assignments: TeacherClassAssignment[] = res.data.data || [];
        const sectionMap = new Map();
        assignments.forEach((a) => {
          if (a.classId?._id === selectedClassId && a.sectionId?._id) {
            sectionMap.set(a.sectionId._id, a.sectionId);
          }
        });
        const sectionList = Array.from(sectionMap.values());
        setSections(sectionList);
        // Do not auto-select first section if one is already selected (e.g. from context)
  ...
  return (
    <div className="space-y-2">
      <div className="bg-gradient-to-r from-indigo-50/90 via-white/80 to-indigo-100/90 backdrop-blur-xl -mx-6 px-6 py-3 flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-indigo-100 shadow-sm sticky top-0 z-20">
        <div>
          <h1 className="text-xl font-bold text-slate-800 tracking-tight">Student Directory</h1>
          <p className="text-xs text-slate-500 font-medium">Access and manage students in your assigned classes.</p>
        </div>
      ...
      <div className="card-clean px-6 py-4 bg-white/50 backdrop-blur-sm border-slate-300">
                      ...
                      {sections.map((s) => (
                        <button
                          ...
                          className={`px-4 py-2 rounded-xl text-xs font-bold transition-all border ${
                            ...
                            : 'bg-slate-50 border-slate-100 text-slate-500 hover:bg-white hover:border-emerald-300 hover:text-emerald-600'
                          }`}
                        >
                          Section {s.name}
                        </button>
              ...
              <div className="space-y-3">
                ...
                <div className="relative">
                  <select
                    ...
                    className="w-full pl-4 pr-10 py-2 bg-white border border-slate-200 rounded-2xl text-xs font-bold text-slate-700 outline-none focus:ring-2 focus:ring-indigo-100 focus:border-indigo-500 transition-all appearance-none cursor-pointer"
                  >
              ...
              <div className="space-y-3">
                ...
                <div className="relative">
                  <select
                    ...
                    className="w-full pl-4 pr-10 py-2 bg-white border border-slate-200 rounded-2xl text-xs font-bold text-slate-700 outline-none focus:ring-2 focus:ring-indigo-100 focus:border-indigo-500 transition-all appearance-none cursor-pointer disabled:opacity-50"
                  >
      ...
      <div className="card-clean bg-white/70 backdrop-blur-sm border-slate-300 overflow-hidden flex flex-col">
        {/* Search Header */}
                  ...
                  <tr key={student._id} className="hover:bg-slate-50/50 transition-colors group">
                    ...
                    <td className="px-6 py-2 text-right">
                        <button 
                          ...
                          className="p-0.5 bg-white border border-slate-200 text-slate-400 rounded-xl hover:bg-slate-900 hover:text-white hover:border-slate-900 transition-all shadow-sm group/btn"
                          title="View Profile"
                        >
